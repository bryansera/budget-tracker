...
function SpendingChart({
  ...
  const chartData = useMemo(() => {
    ...
    const sortedEntries = Object.entries(dataTotals)
      .sort(([, a], [, b]) => b - a);
  ...
  return (
    <Paper elevation={0} sx={{ p: 3, mb: 3, border: '1px solid', borderColor: 'divider' }}>
      ...
      {/* Donut Chart */}
      <ResponsiveContainer width="100%" height={chartHeight} minWidth={600}>
        <PieChart>
          <Pie
            data={chartData}
            cx="50%"
          ...
          >
            {chartData.map((entry, index) => (
              <Cell
                ...
                stroke={selectedItem === entry.name ? '#000' : '#fff'}
                strokeWidth={selectedItem === entry.name ? 3 : 1}
                opacity={selectedItem && selectedItem !== entry.name ? 0.5 : 1}
              />
            ))}
          </Pie>
            ...
            >
              ...
              {chartData.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={COLORS[index % COLORS.length]}
                  stroke={selectedItem === entry.name ? '#000' : 'transparent'}
